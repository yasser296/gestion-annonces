// WishlistButton.js - Version moderne avec animations améliorées
...
const WishlistButton = ({ annonceId, isOwner = false, className = "", size = "md" }) => {
  ...
  const checkWishlistStatus = async () => {
    try {
      ...
      const isInList = response.data.some(item => 
        item.annonce_id._id === annonceId || item.annonce_id === annonceId
      );
  ...
  const toggleWishlist = async (e) => {
    ...
    if (!user) {
      // Rediriger vers la page de connexion ou afficher un toast
      return;
    }
    ...
    try {
      ...
      if (isInWishlist) {
        ...
        await axios.delete(
          `${process.env.REACT_APP_API_URL}/api/wishlist/${annonceId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
